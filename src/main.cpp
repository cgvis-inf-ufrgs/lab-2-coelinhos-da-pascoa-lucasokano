...
int main(int argc, char *argv[])
{
    ...
    GLFWwindow *window;
    window = glfwCreateWindow(800, 600, "INF01047 - 00216888 - Lucas Kazuo Okano", NULL, NULL);
    if (!window)
    {
        glfwTerminate();
...
#define PLANE 2
        ...
        const int NUM_BUNNIES = 12;
        const float raio = 3.0f;
        float tempo = (float)glfwGetTime();
        const float PI = 3.141592f;
        float angle = 0.5f * tempo;
        for (int i = 0; i < NUM_BUNNIES; i++)
        {
            float bunny_angle = angle + (2.0f * PI * i) / NUM_BUNNIES;

            float bunny_x = raio * cosf(bunny_angle);
            float bunny_z = raio * sinf(bunny_angle);
            float bunny_y = 0.0f + 0.5f * sinf(2.0f * tempo + i);
            const int NUM_ELECTRONS = 2;
            const float ELECTRON_RADIUS = 0.45f;
            const float ELECTRON_SPEED = 3.0f;
            for (int j = 0; j < NUM_ELECTRONS; j++)
            {
                float electron_angle = ELECTRON_SPEED * tempo + j * PI;

                float electron_x = bunny_x;
                float electron_y = bunny_y + ELECTRON_RADIUS * cosf(electron_angle);
                float electron_z = bunny_z + ELECTRON_RADIUS * sinf(electron_angle);

                model =
                    Matrix_Translate(electron_x, electron_y, electron_z) * Matrix_Scale(0.08f, 0.08f, 0.08f);
            ...
            {
                model =
                    Matrix_Translate(bunny_x, bunny_y, bunny_z) * Matrix_Rotate_Y(-bunny_angle + PI / 2.0f) * Matrix_Rotate_Z(4.0f * tempo) // vira de ponta cabeça
                    * Matrix_Scale(0.3f, 0.3f, 0.3f);
            }
            else
            {
                model =
                    Matrix_Translate(bunny_x, bunny_y, bunny_z) * Matrix_Rotate_Y(-bunny_angle + PI / 2.0f) * Matrix_Scale(0.3f, 0.3f, 0.3f);
            }
            glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model));
        ...
        model = Matrix_Translate(0.0f, -1.0f, 0.0f) * Matrix_Scale(5.0f, 5.0f, 5.0f);
        glUniformMatrix4fv(g_model_uniform, 1, GL_FALSE, glm::value_ptr(model));
